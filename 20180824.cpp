#if 1
/*
전국 자동차 경주 대회가 매년 열리고 있다. 이 대회에서는 출발지점부터 도착지점까지 거리가 워낙 멀기 때문에 경주 도중에
각 자동차는 정비소를 방문하여 정비를 받아야 한다. 정비소들은 출발지점에서 도착지점으로 가는 길가에 있으며 ①번부터 차례로 번호가 붙어 있다.
...
넷째 줄에는 정비소 별 정비 시간이 차례로 주어진다. 모든 입력은 양의 정수이다.
...
1. 파라미터
 . pos : 정비할 정비소 번호
 . dist : pos 이전까지 정비 안하고 온 거리
 . cost : pos 이전까지의 정비 시간

2. 리턴 : void

3. 종료조건
 . pos == N + 1 : 끝까지 내려오면... cost를 참조하여 sol업데이트

4. 가지치기
 1) dist + 다음 정비소까지 거리(정비 안하는 경우) > K
 2) cost >= sol
...
int K, N, sol = 0x7fffffff;
int L[50 + 10]; //정비 시간
int T[50 + 10]; //정비소의 거리


void DFS(int pos, int dist, int cost) {
